...
def is_fa(transitions):
    ...
    # Check if each state has at most one transition for every input symbol
    for state in states:
        state_transitions = [transition for transition in transitions if transition[0] == state]
        symbols_seen = set()
...
def run():
    result = is_fa(transection_list)
    if(result):
        messagebox.showinfo("Result","This is a Finite Automaton")
    else:
        messagebox.showinfo("Result","This is Not a Finite Automaton")
