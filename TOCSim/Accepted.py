...
def is_accepted(string, transitions, initial_state, accepting_states):
    current_state = initial_state

    for symbol in string:
        found_transition = False
        for transition in transitions:
            state, input_symbol, next_state = transition
...
def run():
    ...
    if(len(input_state)==1 and len(final_state)!=0 and len(transection_list)!=0):
        ists = input_state[0]
        string = askstring('Input', 'Enter the value ')
        result = is_accepted(string,transection_list,ists,set(final_state))
        if(result):
            messagebox.showinfo("Result","String is accepted..")
        else:
            messagebox.showinfo("Result","String is rejected...")
